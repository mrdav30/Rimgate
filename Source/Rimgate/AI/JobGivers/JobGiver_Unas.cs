...
public class JobGiver_Unas : JobGiver_Work
{
    // Basically sets "anything" all the time in their time table
    public override float GetPriority(Pawn pawn) => 5.5f;
    ...
    {
        Pawn_Unas unas = pawn as Pawn_Unas;
        if (unas == null || !(giver.def.nonColonistsCanDo || unas.Faction.IsOfPlayerFaction()))
            return false;

        if (unas.WorkTagIsDisabled(giver.def.workTags))
            return false;

        if (giver.def.workType != null && unas.WorkTypeIsDisabled(giver.def.workType))
            return false;

        if (giver.ShouldSkip(unas))
            return false;

        if (giver.MissingRequiredCapacity(unas) != null)
            return false;

        if (unas.RaceProps.IsMechanoid && !giver.def.canBeDoneByMechs)
            return false;
